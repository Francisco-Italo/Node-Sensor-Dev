...
void acc_comm()
{
    ...
    _accel_pck.xAxis  = RX_Data[5] << 8 | RX_Data[4];
    _accel_pck.yAxis  = RX_Data[3] << 8 | RX_Data[2];
    _accel_pck.zAxis  = RX_Data[1] << 8 | RX_Data[0];
...
void gas_comm()
{
    ...
    SYSCFG0 = FRWPPW | DFWP;            // Program FRAM write enable
    _co2_pck.co2Lvl = (RX_Data[3] << 8 | RX_Data[2]);
    _co2_pck.tvocLvl = (RX_Data[1] << 8 | RX_Data[0]);
    SYSCFG0 = FRWPPW | PFWP | DFWP;     // Program FRAM write protected (not writable)
}
// End of file
